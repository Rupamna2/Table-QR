...
1. Customer scans table QR code (which contains a cryptographically signed signature and timestamp)
2. Next.js Edge Middleware validates the QR signature and establishes a 3-hour secure HttpOnly session cookie (`active_table_session`).
3. Customer browses categorized menu, adds items with variants/notes to cart.
4. Customer reviews cart, selects payment method, and submits order. Conditional security checks apply:
   - Online payments (UPI/Card) skip extra verification.
   - Cash-at-table payments enforce a soft Network/IP geofence matching check to confirm physical presence.
5. Kitchen/owner dashboard receives order in real time.
6. Customer sees live status updates (Received → Preparing → Ready).
7. Owner marks order complete; customer is optionally prompted for a post-order survey/discount.
...
- Physical QR Anchoring (Zero-OTP) using signed URLs
- Visual digital menu with categories, variants, photos, allergens
- Cart with item customization and special notes
- Abstract Payment Interface (IPaymentGateway via Strategy Pattern: Razorpay, Cash, SepoliaCrypto)
...
- Owner sales stats (today's revenue, order count, top items)
- Table QR code generation (with HMAC signatures)
...
- Peak-hour AI analytics

## Scope

### In Scope

- Single-restaurant deployment (multi-tenant is out of scope for MVP)
- Web-based PWA, mobile-first, no native app
- Abstracted Payment Processing (Razorpay, Cash, Crypto)
...
1. A customer can scan a QR code, securely enter an ordering session via cryptographically signed URL, order, pay, and see live status updates.
2. An owner can see a new order appear on the live order board within 2 seconds of submission, without a page refresh.
3. An owner can add/edit/disable a menu item and see it reflected on the customer menu without redeploying.
4. `npm run build` passes at the close of every unit in `00-build-plan.md`.
